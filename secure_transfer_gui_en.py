...
APP_NAME = "Orange Secure Transfer"
...
def unique_target(directory: pathlib.Path, file_name: str) -> pathlib.Path:
    ...
    if not safe_name or safe_name in (".", ".."):
        raise ValueError(ui_text("Invalid file name"))
                ...
                for chunk in iter(lambda: handle.read(FILE_CHUNK_SIZE), b""):
                    ...
                    if now - last_emit >= 0.1 or sent == file_size:
                        self.file_changed.emit({
                            ...
                            "detail": f"{ui_text('Sending')} {human_size(sent)} / {human_size(file_size)}",
                            "hash": digest.hex(),
                        })
                        last_emit = now
            self.file_changed.emit({
    ...
    def _receive_chunk(self, payload: bytes) -> None:
        ...
        if now - self._last_receive_emit >= 0.1 or received == total:
            self.file_changed.emit({
                ...
                "detail": f"{ui_text('Receiving')} {human_size(received)} / {human_size(total)}",
            })
...
class StartDialog(QDialog):
    def __init__(self) -> None:
        ...
        self.start_button = QPushButton(ui_text("Start listening"))
...
class ClickableBubble(QFrame):
    def __init__(self, own: bool) -> None:
        ...
        self.setToolTip(ui_text("Click to copy all information"))
    ...
    def mousePressEvent(self, event) -> None:
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            self.setToolTip(ui_text("Copied"))
...
class MainWindow(QMainWindow):
    ...
    def _on_file_changed(self, event: Dict[str, object]) -> None:
        ...
        if event.get("state") == "complete":
            direction = "Sent file" if event.get("direction") == "out" else "Received file"
    ...
    def _export_history(self) -> None:
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export chat history", default_name, "Text files (*.txt)"
        )
        if not path:
            return
